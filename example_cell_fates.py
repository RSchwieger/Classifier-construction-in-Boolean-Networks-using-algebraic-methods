...
'MPT', 'NFKB1', 'RIP1', 'RIP1K', 'RIP1ub', 'ROS', 'SMAC', 'TNF', 'TNFR', 'XIAP', 'apoptosome', 'cFLIP', 'cIAP']
	...
	#f[CASP8] = ((1+DISCTNF)*(1+DISC-FAS)*CASP3*(1+cFLIP) + (1+DISCTNF)*DISC-FAS*(1+cFLIP) +(1+DISCTNF)*(1+DISC-FAS)*CASP3*(1+cFLIP) + (1+DISCTNF)*DISC-FAS*(1+cFLIP))*DISCTNF*(1+cFLIP) +  ((1+DISCTNF)*(1+DISC-FAS)*CASP3*(1+cFLIP) + (1+DISCTNF)*DISC-FAS*(1+cFLIP) +(1+DISCTNF)*(1+DISC-FAS)*CASP3*(1+cFLIP) + (1+DISCTNF)*DISC-FAS*(1+cFLIP)) + DISCTNF*(1+cFLIP)
        f[CASP8] = DISCTNF*DISC-FAS + DISCTNF*CASP3 + DISCTNF*cFLIP + DISC-FAS*CASP3 + DISC-FAS*cFLIP + CASP3*cFLIP + DISCTNF*DISC-FAS*CASP3 + DISCTNF*DISC-FAS*cFLIP + DISCTNF*CASP3*cFLIP + DISC-FAS*CASP3*cFLIP + DISCTNF*DISC-FAS*CASP3 + DISCTNF + DISC-FAS + CASP3
	f[Cytc] = MOMP
	f[DISC-FAS]  = FASL*FADD
	f[DISCTNF]  = TNFR*FADD
	f[FADD]  = FADD
	f[FASL]  = FASL
	f[IKK]  = RIP1ub
	...
	f[MPT]  = (1+BCL2)*ROS
	f[NFKB1]  = IKK*(1+CASP3)
	# f[RIP1]  = (1+TNFR)*DISC-FAS*(1+CASP8)*TNFR*(1+CASP8) + (1+TNFR)*DISC-FAS*(1+CASP8) + TNFR*(1+CASP8)
        f[RIP1] = TNFR*DISC-FAS + TNFR*CASP8 + DISC-FAS*CASP8 + TNFR*DISC-FAS*CASP8 + TNFR + DISC-FAS
	f[RIP1K]  = RIP1
	f[RIP1ub]  = RIP1*cIAP
